...
class LIMEProcessor:
    ...
    def _ollama_predictor(self, texts: List[str]) -> np.ndarray:
        """Improved Ollama prediction function with confidence scores"""
        import requests
        probs = []
        ...
        for text in texts:
            try:
                ...
                word_overlap = len(words.intersection(response_words))
                
                # Normalize confidence based on word overlap and response relevance
                confidence = min((word_overlap / len(words) if words else 0.5) + 0.2, 1.0)
    ...
    def explain_text(self, text: str, model_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            explanation = self.explainer.explain_instance(
                ...
                num_samples=100  # Increased for better accuracy
            )
