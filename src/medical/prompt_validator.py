...
from src.tracking.logging import log_validation_action, _calculate_edit_distance, _determine_diff_type
...
def add_highlights(sentences, validation_list=None, bg="red", text="red"):
    """Add visual highlights to sentences with customizable colors"""
    # Handle both single string and list inputs
    if isinstance(sentences, str):
        sentences = [sentences]
    
    # Ensure validation_list is properly formed
    if validation_list is None:
        # Default to no highlighting (all valid)
        validation_list = [True] * len(sentences)
    elif isinstance(validation_list, dict):
        # Handle dict input by converting to a list
        validation_list = [True] * len(sentences)
    elif not isinstance(validation_list, list):
        # Convert any other type to a list
        validation_list = [validation_list] * len(sentences)
    
    # Ensure lengths match
    if len(validation_list) < len(sentences):
        validation_list.extend([True] * (len(sentences) - len(validation_list)))
